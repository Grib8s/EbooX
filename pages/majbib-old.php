<div class="card p-3 mb-4"><h3>Livres ajoutés :</h3></div>
...
function selectcoverandcontent($racine) {
	$dp = opendir($racine);
	while($entree = readdir($dp)){
		...
		// traitement pour le fichier trouvé 
			...
			rename($racine.$entree,'../upload/files/infostemp/'.$entree);
		}
		else if (!is_file($racine.$entree)) {
			// c'est un dossier! 
			if( $entree != '.' && $entree != '..') selectcoverandcontent($racine.$entree.'/');
		}
	}
	closedir($dp);
}
		...
		// traitement pour le fichier trouvé 
			...
			if (in_array ($extension, array ('.gif','.jpg','.jpeg','.png'))){
              if (filesize($racine.$entree)>$file["size"]) {
              	...
              	rename($racine.$entree,'../upload/files/infostemp/cover'.$extension);
              	convertImage('../upload/files/infostemp/cover'.$extension);
              }
            }
		}
		else if (!is_file($racine.$entree)) {
			// c'est un dossier! 
			if( $entree != '.' && $entree != '..') findcover($racine.$entree.'/');
		}
	}
	closedir($dp);
}
...
{
    // jpg, png, gif or bmp
    $outputImage="../upload/files/infostemp/cover.jpg";
    $quality="97";
...
function redimcover() {
	if (file_exists("../upload/files/infostemp/cover.jpg")) {
		$img = '../upload/files/infostemp/cover.jpg';
...
function moovebook($livrepath,$livre,$auteur,$titre) {
	if (!file_exists("../Books/".$auteur."/".$titre)) mkdir_r("../Books/".$auteur."/".$titre, 0777);
	rename($livrepath,"../Books/".$auteur."/".$titre."/".$livre);
	if (file_exists("../upload/files/infostemp/cover.jpg")) rename("../upload/files/infostemp/cover.jpg","../Books/".$auteur."/".$titre."/cover.jpg");
...
if (is_dir($dir)) {
   if ($dh = opendir($dir)) {
       while (($file = readdir($dh)) !== false) {
           if( $file != '.' && $file != '..' && preg_match('#\.(epub)$#i', $file)) {
           ...
           //décompresser dans tmp/
			...
			if ($res === TRUE) {
			  ...
			  selectcoverandcontent($dir.'tmp/');
			  //extraire les données du content.opf
			  if (file_exists($dir.'infostemp/content.opf')) {
				  $package = simplexml_load_file($dir.'infostemp/content.opf');
				  ...
				  $query = "SELECT id FROM ".$mysql_prefix."ebooks WHERE auteur=:auteur AND titre=:titre;";
				  $q = $pdo->prepare($query);
				  ...
				  if ($q->rowCount()==0) {
					  ...
					  if (!file_exists($dir."infostemp/cover.jpg")) {
					    findcover($dir.'tmp/');
					  }
					  //redimensionner la couverture
					  redimcover();
					  //transférer dans le rep books/auteur/livre/
					  moovebook($dir.$file,$file,$auteur,$titre);
					  //ajouter dans la bdd
						$date=date('Y-m-d');
						$path=$auteur."/".$titre."/";
					    ...
					    $q2->bindParam('pathfile', $path, PDO::PARAM_STR);
					    $q2->execute();
					  //afficher le livre ajouté
					  //echo $file;
					  showbook($i,$titre,$auteur,$descr,$sujet,$file,$path);
					  $i++;
				  } else {
