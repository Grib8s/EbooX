           ...
           //décompresser dans tmp/
			...
			if ($res === TRUE) {
			  ...
			  if (file_exists($dir.$user.'/'.$pathproc.'/infostemp/content.opf')) {
				  ...
				  //$query = "SELECT id FROM ".$mysql_prefix."ebooks WHERE (auteur=:auteur AND titre=:titre) OR (identifier=:identifier AND identifier!='') OR (pathfile=:pathfile);";
				  $query = "SELECT id FROM ".$mysql_prefix."ebooks WHERE (auteur=:auteur AND titre=:titre) OR (pathfile=:pathfile);";
				  ...
				  $q->bindParam('pathfile', $pathfile, PDO::PARAM_STR);
				  $q->execute();
				  if ($q->rowCount()==0) {
					  ...
					  //echo $file;
					  		$querybid = "SELECT id FROM ".$mysql_prefix."ebooks WHERE id=LAST_INSERT_ID();";
							$qbid = $pdo->prepare($querybid);
							$qbid->execute();
							$book_=$qbid->fetch();
					  
					  
					  showbook($book_['id'],$titre,$auteur,$descr,$sujet,$file,"Books/".$pathfile);
					  $i++;
				  } else {
